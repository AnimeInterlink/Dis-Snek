...
class Client(
    ...
    def get_cog(self, name: str) -> Cog | None:
        ...
        Returns:
            A cog, if found
        """
        if cogs := self.get_cogs(name):
            return cogs[0]
        return None

    def load_cog(self, file_name: str, package: str = None, **load_kwargs) -> None:
        """
    ...
    def drop_cog(self, cog_name: str, **unload_kwargs) -> None:
        ...
        if cog := self.get_cogs(cog_name):
            ...
            raise CogLoadException(f"Unable to drop cog: No cog exists with name: `{cog_name}`")
    ...
    def reload_cog(
        ...
        self.load_cog(cog_name, **load_kwargs)
